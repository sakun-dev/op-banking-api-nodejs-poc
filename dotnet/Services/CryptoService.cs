using System.Security.Cryptography;
using System.Text;

namespace dotnet.Services;

public class CryptoService 
{
    public CryptoService() { }
    public string CreateSha256Hash(string content)
    {
        // value: "test"
        // output: 9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08

        using var mySHA256 = SHA256.Create();
        var bytes = Encoding.Default.GetBytes(content);
        byte[] hashValue = mySHA256.ComputeHash(bytes);
        // Console.WriteLine("hashBytes:" + ToHex(hashValue));
        
        return ToHex(hashValue);
    }

    public string SignMessage(string privateKey, string message)
    {
        var bytes = Encoding.Default.GetBytes(message);
        var openSsl = new RSAOpenSsl();

        // https://learn.microsoft.com/en-us/dotnet/api/system.security.cryptography.rsaopenssl.encrypt?view=net-6.0&viewFallbackFrom=net-7.0
        var encrypted = openSsl.Encrypt(bytes, RSAEncryptionPadding.OaepSHA256);

        return ToHex(encrypted);
    }

    private static string ToHex(byte[] bytes)
    {
        StringBuilder result = new StringBuilder(bytes.Length * 2);
        for (int i = 0; i < bytes.Length; i++)
            result.Append(bytes[i].ToString("x2"));
        return result.ToString();
    }

    // TODO: Create unit test
    public static void Test()
    {
        var instance = new CryptoService();
        CreateSha256Hash_Test(instance);
        Sign_Test(instance);
    }

    public static void CreateSha256Hash_Test(CryptoService instance) 
    {
        string input = "test";
        string expectedOutput = "9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08";

        string actualOutput = instance.CreateSha256Hash(input);

        Assert(expectedOutput != actualOutput, @$"The hash is not calculated correctly. 
            
                     - input  : ""{input}""
            Expected - output : ""{expectedOutput}""
            Actual   - output : ""{actualOutput}""
", $"input '{input}' is calculated correctly");
    }

    public static void Sign_Test(CryptoService instance)
    {
        string expectedSignature = "";
        string privateKey = @"-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----";
        string message = @"POST
application/json
Wed, 04 Jan 2023 18:07:13 GMT
c3efaaee-a6bb-449f-9213-e45e23a462ef
6c18c234b1b18b1d97c7043e2e41135c293d0da9
F64FEC49-82BD-496A-B337-ADAB97EE3B05
3B640BD5-E9C6-4A0A-AEAB-6B697B80F27F
https://sandbox.apis.op.fi/paymentbutton/v1/payments
{""amount"":""1.00"",""cancel"":{""url"":""https://shop.example.com/cancel/path""},""reject"":{""url"":""https://shop.example.com/reject/path""},""return"":{""url"":""https://shop.example.com/return/path""},""currency"":""EUR"",""accountId"":""8a5d4aac-8f8f-47ed-ae2f-36ffeaf57c79"",""reference"":""RF3517834735""}";
    
        string actualSignature = instance.SignMessage(privateKey, message);

        Assert(expectedSignature != actualSignature, @$"The signature is not calculated correctly. 
            
            Expected - signature : ""{expectedSignature}""
            Actual   - signature : ""{actualSignature}""
", "Signature is calculated correctly");
    }

    private static void Assert(bool conditionToFail, string errorMessage, string successMessage)
    {
        if(conditionToFail)
        {
            throw new Exception(errorMessage);
        }
        else if (!string.IsNullOrEmpty(successMessage))
        {
            Console.WriteLine(successMessage);
        }
    }
}